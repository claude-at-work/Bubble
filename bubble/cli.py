...
def cmd_run(args: argparse.Namespace) -> int:
    ...
    and serves them from the vault (or PyPI on miss-of-miss). The script
    runs in-process so faults are transparent.
    """
    db.init_db()
...
def cmd_up(args: argparse.Namespace) -> int:
    ...
    # Stage 3: fetch missing
    if plan.missing and not args.no_fetch:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    up.add_argument("--no-fetch", action="store_true",
                    help="don't auto-fetch missing packages from PyPI")
    ...
    run.add_argument("--no-fetch", action="store_true",
                     help="don't auto-fetch from PyPI on vault miss")
