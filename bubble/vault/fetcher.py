...
Wheels are downloaded and unpacked into the vault. sdists fall back to
`pip download` if pip is on PATH; otherwise we error.
"""
    ...
    try:
        if pick["kind"] == "wheel":
            ...
            if not shutil.which("pip3") and not shutil.which("pip"):
                raise RuntimeError(
                    f"{name}=={version} only available as sdist; pip not on PATH"
                )
